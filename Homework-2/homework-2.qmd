...
```{r}
myattempt <- c((sum(replicate(1000,myattempts(0.5)))/1000))
rgeom <- c((sum(replicate(1000,rgeom(1,0.5)))/1000))
dgeom <- c((sum(replicate(1000,dgeom(1,0.5)))/1000))
df <- data.frame(myattempt,rgeom,dgeom)
df
...
plot_binomial_normal_cdf <- function(n, p) {
  # Generate values for the x-axis
  x <- 0:n

  # Compute binomial CDF
  binomial_cdf <- pbinom(x, size = n, prob = p)

  # Compute normal CDF parameters
  mu <- n * p
  sigma <- sqrt(n * p * (1 - p))

  # Compute normal CDF
  normal_cdf <- pnorm(x, mean = mu, sd = sigma)

  # Plotting
  plot(x, binomial_cdf, type = "s", main = "Binomial vs Normal CDF",
       xlab = "Number of Successes", ylab = "Cumulative Probability",
       col = "blue", lty = 2, lwd = 2)
  lines(x, normal_cdf, type = "l", col = "red", lty = 1, lwd = 2)
  legend("right", legend = c("Binomial", "Normal"), col = c("blue", "red"), lty = 1:2, lwd = 2)
}

# Case 1: np and n(1-p) are both large
plot_binomial_normal_cdf(n = 50, p = 0.6)

# Case 2: np is large but n(1-p) < 10
plot_binomial_normal_cdf(n = 30, p = 0.8)

# Case 3: np < 10 and n(1-p) < 10
plot_binomial_normal_cdf(n = 10, p = 0.3)
...
data(exam_grades)
cleaned_data <- na.omit(exam_grades)
head(cleaned_data)

par(mfrow = c(2,2))


qqnorm(cleaned_data$exam1)
qqline(cleaned_data$exam1, col = 2)

qqnorm(cleaned_data$exam2)
qqline(cleaned_data$exam2, col = 2)

qqnorm(cleaned_data$exam3)
qqline(cleaned_data$exam3, col = 2)

qqnorm(cleaned_data$course_grade)
qqline(cleaned_data$course_grade, col = 2)
```
